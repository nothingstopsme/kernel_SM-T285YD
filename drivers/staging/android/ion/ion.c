				     ...
				     unsigned long flags)
{
	...
	//buffer = kzalloc(sizeof(struct ion_buffer), GFP_KERNEL|__GFP_REPEAT );
	if (!buffer)
...
static long ion_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	{
		printk("%s: error, _IOC_SIZE(cmd) %u != sizeof(data) %u", __func__, _IOC_SIZE(cmd), sizeof(data));
		return -EINVAL;
	}
	...
	if (dir & _IOC_WRITE)
		...
		{
			printk("%s: copy_from_user failed", __func__);
			return -EFAULT;
		}
